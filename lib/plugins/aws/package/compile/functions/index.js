...
class AwsCompileFunctions {
  ...
  compileFunctionDestinations(functionName) {
    ...
    if (destinations.onSuccess) {
      ...
      destinationConfig.OnSuccess = { Destination: destinations.onSuccess };
    }
    if (destinations.onFailure) {
      this.ensureTargetExecutionPermission(destinations.onFailure);
      destinationConfig.OnFailure = { Destination: destinations.onFailure };
    }
  ...
  ensureTargetExecutionPermission(arn) {
    ...
    if (isLambdaArn(arn)) {
      ...
      throw new this.serverless.classes.Error(`Unsupported lambda destination arn: ${arn}`);
    }
  }
